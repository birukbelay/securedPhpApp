...
if(isset($_POST['submit'])){
	...
	if (empty($first) || empty($last) || empty($user) || empty($pwd)) {
		header("Location: ../../add-technecian.php?signup=empty!");
		...
		if(!preg_match("/^[a-zA-Z]*$/", $first) || !preg_match("/^[a-zA-Z]*$/", $last)) {
			header("Location: ../../add-technecian.php?signup=invalid");
			exit();
		} else {
			...
			if ($resultCheck > 0) {
				...
				header("Location: ../../add-technecian.php?signup=usertaken");
				exit();
			} else {
					...
					if($fileError==0){
						...
						if($fileSize < 2000000){
							...
							if(!mysqli_stmt_prepare($stmt,$sql)){
								...
								header("Location: ../../add-technecian.php?upload=success");
								} else {
									echo "Error: " . $sql . "<br>" . mysqli_error($conn);
								// echo "Error updating record: " . $conn->error;
								header("Location: ../../add-technecian.php?upload=sqlerror");
								}
							...
							header("Location: ../../add-technecian.php?upload=fileToBig");

							exit();
						}
					}else{
						header("Location: ../../add-technecian.php?upload=notknownerror");

						exit();
					}
				}else{
					header("Location: ../../add-technecian.php?upload=uploadProperFile");
	...
	echo"no submit";
	header("Location: ../../add-technecian.php?upload=noSubmit");
	exit();
}